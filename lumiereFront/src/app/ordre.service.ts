...
export class OrdreService {
  ...
  afficher(): Observable<any[]> {
    return this.http.get<any[]>('http://localhost:8090/ordres', { headers: this.getAuthHeaders() });
  }
  ajouter(depence: any): Observable<any[]> {
    return this.http.post<any[]>('http://localhost:8090/ordres', depence, { headers: this.getAuthHeaders() });
  }
  ...
  supprimer(id: number): Observable<any> {
    return this.http.delete<any>(`http://172.18.3.125:8090/ordres/${id}`, { headers: this.getAuthHeaders() });
  }

  
  confirmer(id: number): Observable<void> {
    return this.http.put<any>(`http://172.18.3.125:8090/ordres/confirmer/${id}`,{}, { headers: this.getAuthHeaders() });
  }
  

  afficheremail(id:any):Observable<any> {
    return this.http.get<any>('http://172.18.3.125:8090/clients/email/${id}', { headers: this.getAuthHeaders() });

  }
  private baseUrl = 'http://172.18.3.125:8090/clients';
