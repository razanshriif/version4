...
export class HomePage implements OnInit {
  ...
  loadMyStats() {
    ...
    this.http.get(`${baseUrl}/client/dashboard/stats`, { headers }).subscribe({
      next: (res: any) => {
        this.stats = res;
  ...
  loadMesDemandesRecentes() {
    const headers = this.getAuthHeaders();
    const baseUrl = environment.apiUrl.replace('/api', '');
    this.http.get(`${baseUrl}/client/dashboard/mes-demandes/recentes`, { headers }).subscribe({
      next: (res: any) => {
  ...
  loadMesLivraisonsActives() {
    const headers = this.getAuthHeaders();
    const baseUrl = environment.apiUrl.replace('/api', '');
    this.http.get(`${baseUrl}/client/dashboard/mes-livraisons/actives`, { headers }).subscribe({
      next: (res: any) => {
