...
export class OrdreService {
  ...
  afficher(): Observable<any[]> {
    return this.http.get<any[]>('http://localhost:8090/api/ordres', { headers: this.getAuthHeaders() });
  }
  ajouter(depence: any): Observable<any[]> {
    return this.http.post<any[]>('http://localhost:8090/api/ordres', depence, { headers: this.getAuthHeaders() });
  }
  ...
  supprimer(id: number): Observable<any> {
    return this.http.delete<any>(`http://localhost:8090/api/ordres/${id}`, { headers: this.getAuthHeaders() });
  }


  confirmer(id: number): Observable<void> {
    return this.http.put<any>(`http://localhost:8090/api/ordres/confirmer/${id}`, {}, { headers: this.getAuthHeaders() });
  }


  afficheremail(id: any): Observable<any> {
    return this.http.get<any>('http://localhost:8090/api/clients/email/${id}', { headers: this.getAuthHeaders() });

  }
  private apiUrl = 'http://localhost:8090/api/ordres';
